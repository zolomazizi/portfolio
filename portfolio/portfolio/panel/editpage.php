...
if($_SERVER['REQUEST_METHOD'] == "POST"){ 
    ...
    /** Traitement Table Work */
        try{
            if(isset($_POST['title']) && isset($_POST['img']) && isset($_POST['des']) && isset($_POST['catg'])){
                ...
                if(!empty($img)){
                    $query = "update items set title=$title,des=$des,catg=$catg,img='$img' where id_items=$id_items";

                }else{
                    $query = "update items set title=$title,des=$des,catg=$catg where id_items=$id_items";
...
$select = $db->query('SELECT id_items, title, des, catg, img FROM items where id_items = '.Session::get('id_items_edit'));
        ...
        <nav class="navbar navbar-light border-bottom border-danger bg fixed-top">
            <div class="container">
                <a class="navbar-brand" href="#">
                    <span id="name"><i class="fas fa-hamburger"></i> Fast-Food</span></a>
        ...
        <form method="POST" action="" class="add-form container">
            ...
            <div class="form-group">
                    ...
                    <select class="form-control" name="catg">
                            ...
                            <option value="sandwichs">HTML5 CSS3</option>
                            <option value="pizza">Java Script</option>
                            <option value="drinks">PHP</option>
                    </select>
            </div>
